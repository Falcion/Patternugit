# References

> [!Note]
> References were partially script-generated (packages references), if you see any
> mistake (skipped reference, wrong license and etc.) please, open an documentation
> issue.
> **Last edited: 20th January, 2025**

This document is created to reference every project, repository, organization upon
which this one heavily relies, meaning, giving respects to them:

## Externals

- [Shields.io (CC0-1.0 license)](https://shields.io/)
- [simpleicons.org (CC0-1.0 license)](https://simpleicons.org/)
- [super-linter (MIT)](https://github.com/super-linter/super-linter)

**Special thanks to other projects not specified here, but which are part of this
entire chain of dependencies which develop this infrastructure.**

## Packages (NPM)

### MIT

- [@babel/eslint-parser](https://www.npmjs.com/package/@babel/eslint-parser)
- [@changesets/cli](https://www.npmjs.com/package/@changesets/cli)
- [@commitlint/cli](https://www.npmjs.com/package/@commitlint/cli)
- [@commitlint/config-conventional](https://www.npmjs.com/package/@commitlint/config-conventional)
- [@electron/lint-roller](https://www.npmjs.com/package/@electron/lint-roller)
- [@eslint/eslintrc](https://www.npmjs.com/package/@eslint/eslintrc)
- [@eslint/js](https://www.npmjs.com/package/@eslint/js)
- [@jscpd/badge-reporter](https://www.npmjs.com/package/@jscpd/badge-reporter)
- [@types/fs-extra](https://www.npmjs.com/package/@types/fs-extra)
- [@types/minimist](https://www.npmjs.com/package/@types/minimist)
- [@types/node](https://www.npmjs.com/package/@types/node)
- [@types/SemVer](https://www.npmjs.com/package/@types/semver)
- [@typescript-eslint/eslint-plugin](https://www.npmjs.com/package/@typescript-eslint/eslint-plugin)
- [@typescript-eslint/parser](https://www.npmjs.com/package/@typescript-eslint/parser)
- [builtin-modules](https://www.npmjs.com/package/builtin-modules)
- [chalk](https://www.npmjs.com/package/chalk)
- [check-for-leaks](https://www.npmjs.com/package/check-for-leaks)
- [colors](https://www.npmjs.com/package/colors)
- [dugite](https://www.npmjs.com/package/dugite)
- [EditorConfig](https://www.npmjs.com/package/editorconfig)
- [esbuild](https://www.npmjs.com/package/esbuild)
- [esbuild-plugin-node-externals](https://www.npmjs.com/package/esbuild-plugin-node-externals)
- [esbuild-sass-plugin](https://www.npmjs.com/package/esbuild-sass-plugin)
- [ESLint](https://www.npmjs.com/package/eslint)
- [eslint-config-prettier](https://www.npmjs.com/package/eslint-config-prettier)
- [eslint-plugin-import](https://www.npmjs.com/package/eslint-plugin-import)
- [eslint-plugin-jsonc](https://www.npmjs.com/package/eslint-plugin-jsonc)
- [eslint-plugin-markdownlint](https://www.npmjs.com/package/eslint-plugin-markdownlint)
- [eslint-plugin-node](https://www.npmjs.com/package/eslint-plugin-node)
- [eslint-plugin-prettier](https://www.npmjs.com/package/eslint-plugin-prettier)
- [fs-extra](https://www.npmjs.com/package/fs-extra)
- [github-wiki-sidebar](https://www.npmjs.com/package/github-wiki-sidebar)
- [globals](https://www.npmjs.com/package/globals)
- [husky](https://www.npmjs.com/package/husky)
- [imagemin-lint-staged](https://www.npmjs.com/package/imagemin-lint-staged)
- [jscpd](https://www.npmjs.com/package/jscpd)
- [lint-staged](https://www.npmjs.com/package/lint-staged)
- [markdownlint](https://www.npmjs.com/package/markdownlint)
- [markdownlint-cli](https://www.npmjs.com/package/markdownlint-cli)
- [markdownlint-cli2](https://www.npmjs.com/package/markdownlint-cli2)
- [minimist](https://www.npmjs.com/package/minimist)
- [node-fetch](https://www.npmjs.com/package/node-fetch)
- [pre-flight](https://www.npmjs.com/package/pre-flight)
- [prettier](https://www.npmjs.com/package/prettier)
- [remark](https://www.npmjs.com/package/remark)
- [remark-cli](https://www.npmjs.com/package/remark-cli)
- [remark-preset-lint-markdown-style-guide](https://www.npmjs.com/package/remark-preset-lint-markdown-style-guide)
- [shellcheck](https://www.npmjs.com/package/shellcheck)
- [shx](https://www.npmjs.com/package/shx)
- [standard](https://www.npmjs.com/package/standard)
- [stylelint](https://www.npmjs.com/package/stylelint)
- [textlint](https://www.npmjs.com/package/textlint)
- [textlint-rule-date-weekday-mismatch](https://www.npmjs.com/package/textlint-rule-date-weekday-mismatch)
- [textlint-rule-editorconfig](https://www.npmjs.com/package/textlint-rule-editorconfig)
- [textlint-rule-footnote-order](https://www.npmjs.com/package/textlint-rule-footnote-order)
- [textlint-rule-no-curly-quotes](https://www.npmjs.com/package/textlint-rule-no-curly-quotes)
- [textlint-rule-no-todo](https://www.npmjs.com/package/textlint-rule-no-todo)
- [textlint-rule-period-in-header](https://www.npmjs.com/package/textlint-rule-period-in-header)
- [textlint-rule-terminology](https://www.npmjs.com/package/textlint-rule-terminology)
- [ts-loader](https://www.npmjs.com/package/ts-loader)
- [ts-node](https://www.npmjs.com/package/ts-node)
- [ts-standard](https://www.npmjs.com/package/ts-standard)

### (MIT AND BSD)

- [dictionary-en](https://www.npmjs.com/package/dictionary-en)

### BSD-2-Clause

- [dotenv](https://www.npmjs.com/package/dotenv)

### Zlib

- [esbuild-plugin-glsl](https://www.npmjs.com/package/esbuild-plugin-glsl)

### Apache-2.0

- [lint](https://www.npmjs.com/package/lint)
- [TypeScript](https://www.npmjs.com/package/typescript)

### ISC

- [@npmcli/package-json](https://www.npmjs.com/package/@npmcli/package-json)
- [eslint-plugin](https://www.npmjs.com/package/eslint-plugin)
- [SemVer](https://www.npmjs.com/package/semver)
- [standard-version](https://www.npmjs.com/package/standard-version)

### GPL-3.0-or-later

- [textlint-rule-spelling](https://www.npmjs.com/package/textlint-rule-spelling)

## Packages (Python)

### LGPL-2.1-or-later

- [astroid](https://pypi.org/project/astroid/)

### Unknown

- [Jinja2](https://pypi.org/project/Jinja2/)
- [attrs](https://pypi.org/project/attrs/)
- [backcall](https://pypi.org/project/backcall/)
- [build](https://pypi.org/project/build/)
- [click](https://pypi.org/project/click/)
- [colorama](https://pypi.org/project/colorama/)
- [idna](https://pypi.org/project/idna/)
- [jsonschema-specifications](https://pypi.org/project/jsonschema-specifications/)
- [packaging](https://pypi.org/project/packaging/)
- [pathspec](https://pypi.org/project/pathspec/)
- [platformdirs](https://pypi.org/project/platformdirs/)
- [prompt_toolkit](https://pypi.org/project/prompt_toolkit/)
- [pyproject_hooks](https://pypi.org/project/pyproject_hooks/)
- [referencing](https://pypi.org/project/referencing/)
- [rpds-py](https://pypi.org/project/rpds-py/)
- [setuptools](https://pypi.org/project/setuptools/)
- [soupsieve](https://pypi.org/project/soupsieve/)
- [tinycss2](https://pypi.org/project/tinycss2/)
- [typing_extensions](https://pypi.org/project/typing_extensions/)
- [urllib3](https://pypi.org/project/urllib3/)
- [wheel](https://pypi.org/project/wheel/)

### MIT License

- [beautifulsoup4](https://pypi.org/project/beautifulsoup4/)
- [mypy-extensions](https://pypi.org/project/mypy-extensions/)
- [PyYAML](https://pypi.org/project/PyYAML/)
- [black](https://pypi.org/project/black/)
- [charset-normalizer](https://pypi.org/project/charset-normalizer/)
- [docopt](https://pypi.org/project/docopt/)
- [executing](https://pypi.org/project/executing/)
- [flake8](https://pypi.org/project/flake8/)
- [isort](https://pypi.org/project/isort/)
- [jedi](https://pypi.org/project/jedi/)
- [jsonschema](https://pypi.org/project/jsonschema/)
- [lib](https://pypi.org/project/lib/)
- [mypy](https://pypi.org/project/mypy/)
- [parso](https://pypi.org/project/parso/)
- [pickleshare](https://pypi.org/project/pickleshare/)
- [pure_eval](https://pypi.org/project/pure_eval/)
- [pycodestyle](https://pypi.org/project/pycodestyle/)
- [pyflakes](https://pypi.org/project/pyflakes/)
- [pyink](https://pypi.org/project/pyink/)
- [ruff](https://pypi.org/project/ruff/)
- [six](https://pypi.org/project/six/)
- [stack-data](https://pypi.org/project/stack-data/)
- [tomlkit](https://pypi.org/project/tomlkit/)
- [wcwidth](https://pypi.org/project/wcwidth/)
- [yarg](https://pypi.org/project/yarg/)

### Apache Software License

- [bleach](https://pypi.org/project/bleach/)

### MPL-2.0

- [certifi](https://pypi.org/project/certifi/)

### new BSD License

- [decorator](https://pypi.org/project/decorator/)

### PSFL

- [defusedxml](https://pypi.org/project/defusedxml/)

### BSD-3-Clause

- [dill](https://pypi.org/project/dill/)
- [ipython](https://pypi.org/project/ipython/)
- [mistune](https://pypi.org/project/mistune/)
- [pandocfilters](https://pypi.org/project/pandocfilters/)

### BSD

- [fastjsonschema](https://pypi.org/project/fastjsonschema/)
- [pip-tools](https://pypi.org/project/pip-tools/)
- [webencodings](https://pypi.org/project/webencodings/)

### BSD 3-Clause License - Copyright (c) 2001-2015, IPython Development Team

- [jupyter_client](https://pypi.org/project/jupyter_client/)
- [nbconvert](https://pypi.org/project/nbconvert/)
- [nbformat](https://pypi.org/project/nbformat/)

### BSD 3-Clause License - Copyright (c) 2015-, Jupyter Development Team

- [jupyter_core](https://pypi.org/project/jupyter_core/)

### Copyright (c) 2015 Project Jupyter Contributors

- [jupyterlab_pygments](https://pypi.org/project/jupyterlab_pygments/)

### Copyright 2010 Pallets

- [MarkupSafe](https://pypi.org/project/MarkupSafe/)

### BSD 3-Clause License Copyright (c) 2019-2022, IPython Development Team

- [matplotlib-inline](https://pypi.org/project/matplotlib-inline/)

### Expat license

- [mccabe](https://pypi.org/project/mccabe/)

### BSD 3-Clause License Copyright (c) 2020-, Jupyter Development Team

- [nbclient](https://pypi.org/project/nbclient/)

### Apache-2.0

- [pipreqs](https://pypi.org/project/pipreqs/)
- [requests](https://pypi.org/project/requests/)
- [tornado](https://pypi.org/project/tornado/)
- [types-PyYAML](https://pypi.org/project/types-PyYAML/)
- [asttokens](https://pypi.org/project/asttokens/)

### BSD-2-Clause

- [Pygments](https://pypi.org/project/Pygments/)

### GPL-2.0-or-later

- [pylint](https://pypi.org/project/pylint/)

### Dual License

- [python-dateutil](https://pypi.org/project/python-dateutil/)

### PSF

- [pywin32](https://pypi.org/project/pywin32/)

### BSD 3-Clause License Copyright (c) 2009-2012, Brian Granger, Min Ragan-Kelley

- [pyzmq](https://pypi.org/project/pyzmq/)

### BSD 3-Clause License - Copyright (c) 2001-, IPython Development Team

- [traitlets](https://pypi.org/project/traitlets/)
