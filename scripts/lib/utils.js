...
export async function getCurrentBranch(gitDir) {
    ...
    let branch = await handleGitCall(
    ...
    if (
        ...
        const branches = (
            await handleGitCall(
                ['branch', '--contains', lastCommit, '--remote'],
                gitDir,
            )
        ).split('\n');
...
export function chunkFilenames(filenames, offset = 0) {
    ...
    return filenames.reduce(
        (chunkedFilenames, filename) => {
            ...
            if (
                currChunkLength + filename.length + 1 >
                MAX_FILENAME_ARGS_LENGTH
            ) {
...
export async function findMatchingFiles(top, test) {
    return fs.promises
        .readdir(top, { encoding: 'utf8', recursive: true })
        .then((files) => {
            return files
