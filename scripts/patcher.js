...
import { parse, valid, clean, rcompare } from 'semver';
...
function execGitCommand(gitCommand) {
    ...
    if (output.code !== 0) {
        console.error(
            `Error: git command "${gitCommand}" failed: \n\n ${output.stderr}`,
        );
        process.exit(1);
    }
    return output;
...
function getCommitInfoAsString(version, commitInfo) {
    const formattedVersion =
        version === initialVersion ? version : `${version}+`;
...
function listFeatures(commitsMap) {
    return Array.from(commitsMap.keys()).reduce((result, key) => {
        if (
            key.startsWith('feat') &&
            !shouldIgnoreCommit(key, ignoreFeatureCheckPatterns)
        ) {
