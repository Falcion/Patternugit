...
import { chunkFilenames, findMatchingFiles } from './lib/utils.js'
...
const PROJECT_ROOT = path.normalize(path.dirname(__dirname))
...
const IGNORELIST = new Set(
  ...
  ].map((tokens) => path.join(PROJECT_ROOT, ...tokens))
)
...
function cpplint(args) {
  ...
  const result = childProcess.spawnSync(IS_WINDOWS ? 'cpplint.bat' : 'cpplint.py', args, {
    encoding: 'utf8',
    shell: true
  })
...
const LINTERS = [
  {
    ...
    roots: ['shell'],
    test: (filename) =>
      filename.endsWith('.cc') || (filename.endsWith('.h') && !isObjCHeader(filename)),
    run: (opts, filenames) => {
  ...
  },
  {
    ...
    roots: ['shell'],
    test: (filename) =>
      filename.endsWith('.mm') || (filename.endsWith('.h') && isObjCHeader(filename)),
    run: (opts, filenames) => {
      const clangFormatFlags = opts.fix ? ['--fix'] : []
      spawnAndCheckExitCode('python3', [
        'script/run-clang-format.py',
  ...
  },
  {
    ...
    run: (opts, filenames) => {
      ...
      const env = {
        PYTHONPATH: path.join(PROJECT_ROOT, 'script'),
        ...process.env
      }
  ...
  },
  {
    ...
    run: (opts, filenames) => {
      const allOk = filenames
        .map((filename) => {
          const env = {
            CHROMIUM_BUILDTOOLS_PATH: path.resolve(PROJECT_ROOT, '..', 'buildtools'),
...
function parseCommandLine() {
  ...
  if (help || opts.help) {
    console.log(
      'Usage: script/lint.js [--cc] [--js] [--py] [-c|--changed] [-h|--help] [-v|--verbose] [--fix] [--only -- file1 file2]'
    )
    process.exit(0)
  }
  return opts
...
async function findFiles(args, linter) {
  ...
  if (args.changed) {
    includelist = await findChangedFiles(PROJECT_ROOT)
    if (!includelist.size) {
      return []
    }
  } else if (args.only) {
  ...
  for (const root of linter.roots) {
    const files = await findMatchingFiles(path.join(PROJECT_ROOT, root), linter.test)
    filenames.push(...files)
  }

  for (const ignoreRoot of linter.ignoreRoots || []) {
    const ignorePath = path.join(PROJECT_ROOT, ignoreRoot)
  ...
  // to apply.
  return filenames.map((x) => path.relative(PROJECT_ROOT, x))
}
...
async function main() {
  ...
  for (const linter of linters) {
    ...
    if (filenames.length) {
      if (opts.verbose) {
        console.log(
          `linting ${filenames.length} ${linter.key} ${filenames.length === 1 ? 'file' : 'files'}`
        )
      }
