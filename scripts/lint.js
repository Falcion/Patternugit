...
const LINTERS = [
    ...
    },
    {
        ...
        roots: ['.'],
        test: (filename) =>
            filename.endsWith('.gn') || filename.endsWith('.gni'),
        run: (opts, filenames) => {
    ...
    },
    {
        ...
        run: (opts, filenames) => {
            ...
            if (!fs.existsSync(patchesConfig)) {
                console.error(
                    `Patches config file: "${patchesConfig}" does not exist`
                )
                process.exit(1)
            }
            ...
            for (const key of Object.keys(config)) {
                ...
                const dotPatchesPath = path.resolve(
                    targetPatchesDir,
                    '.patches'
                )
                if (!fs.existsSync(dotPatchesPath)) {
                    console.error(
                        `.patches file: "${dotPatchesPath}" does not exist`
                    )
                    process.exit(1)
                }
